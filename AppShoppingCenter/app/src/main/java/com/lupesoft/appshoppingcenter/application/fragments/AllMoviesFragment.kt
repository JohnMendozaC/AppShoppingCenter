                    ...
                    viewModel.addMovie(movieId).observe(viewLifecycleOwner, Observer { result ->
                        when (result.status) {
                            ...
                            Status.ERROR -> {
                                listMovie = null
                                loader.isLoading = true
                                Toast.makeText(context, "Error no se pudo agregar la pelicula", Toast.LENGTH_LONG)
                                    .show()
                            }
                        }
                    })
                    ...
                    viewModel.deleteMovie(movieId).observe(viewLifecycleOwner, Observer { result ->
                        when (result.status) {
                            ...
                            Status.ERROR -> {
                                listMovie = null
                                loader.isLoading = true
                                Toast.makeText(context, "Error no se pudo eliminar la pelicula", Toast.LENGTH_LONG)
                                    .show()
                            }
                        }
                    })
